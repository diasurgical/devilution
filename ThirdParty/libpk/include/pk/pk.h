...
typedef struct
{
    ...
    unsigned short
        phash_to_index[0x900]; // 0DC8: Array of indexes (one for each PAIR_HASH) to the "pair_hash_offsets" table
...
#define CMP_BUFFER_SIZE                                                                                                \
    sizeof(TCmpStruct) // Size of compression structure.
...
typedef struct
{
    ...
    unsigned int (*read_buf)(
        char* buf,
        unsigned int* size,
        void* param); // Pointer to function that reads data from the input stream
    void (*write_buf)(
        char* buf,
        unsigned int* size,
        void* param); // Pointer to function that writes data to the output stream
...
#define EXP_BUFFER_SIZE                                                                                                \
    sizeof(TDcmpStruct) // Size of decompression structure
