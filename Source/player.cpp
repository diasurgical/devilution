...
int __fastcall CalcStatDiff(int pnum)
{
	...
	return MaxStats[c][ATTRIB_STR] - plr[pnum]._pBaseStr + MaxStats[c][ATTRIB_MAG] - plr[pnum]._pBaseMag + MaxStats[c][ATTRIB_DEX] - plr[pnum]._pBaseDex + MaxStats[c][ATTRIB_VIT] - plr[pnum]._pBaseVit;
}
...
BOOL __fastcall PM_DoSpell(int pnum)
{
	...
	if (plr[pnum]._pVar8 == plr[pnum]._pSFNum)
	{
		...
		if (!plr[pnum]._pSplFrom)
		{
			...
			{
				if (!(plr[pnum]._pScrlSpells64 & (unsigned __int64)1 << (plr[pnum]._pRSpell - 1)))
				{
					plr[pnum]._pRSpell = SPL_INVALID;
					plr[pnum]._pRSplType = RSPLTYPE_INVALID;
					drawpanflag = 255;
			...
			{
				if (!(plr[pnum]._pISpells64 & (unsigned __int64)1 << (plr[pnum]._pRSpell - 1)))
				{
					plr[pnum]._pRSpell = SPL_INVALID;
					plr[pnum]._pRSplType = RSPLTYPE_INVALID;
					drawpanflag = 255;
...
void __fastcall CheckPlrSpell()
{
	...
	if (pcurs != CURSOR_HAND || MouseY >= 352 || (chrflag && MouseX < 320 || invflag && MouseX > 320) && rspell != SPL_HEAL && rspell != SPL_IDENTIFY && rspell != SPL_REPAIR && rspell != SPL_INFRA && rspell != SPL_RECHARGE)
	{
		return;
	}
