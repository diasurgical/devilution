...
void __fastcall DrawMissile(int x, int y, int sx, int sy, int a5, int dir, BOOL pre)
{
	int i, mx, my, nCel;
	MissileStruct *m;
	BYTE *pCelBuff;
	...
	if (dMissile[x][y] == -1) {
		for (i = 0; i < nummissiles; i++) {
			...
			if (m->_mix == x && m->_miy == y && m->_miPreFlag == pre && m->_miDrawFlag) {
				...
				my = sy + m->_miyoff;
				if (m->_miUniqTrans)
					Cl2DecodeFrm3(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir, m->_miUniqTrans + 3);
				else if (m->_miLightFlag)
					Cl2DecodeLightTbl(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir);
				else
					Cl2DecodeFrm1(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir);
			}
		}
	} else {
...
void __fastcall DrawClippedMissile(int x, int y, int sx, int sy, int a5, int dir, BOOL pre)
{
	int i, mx, my, nCel;
	MissileStruct *m;
	BYTE *pCelBuff;
	...
	if (dMissile[x][y] == -1) {
		for (i = 0; i < nummissiles; i++) {
			...
			if (m->_mix == x && m->_miy == y && m->_miPreFlag == pre && m->_miDrawFlag) {
				...
				my = sy + m->_miyoff;
				if (m->_miUniqTrans)
					Cl2DecodeFrm5(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir, m->_miUniqTrans + 3);
				else if (m->_miLightFlag)
					Cl2DecodeFrm6(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir);
				else
					Cl2DecodeFrm4(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth, a5, dir);
			}
		}
	} else {
		m = &missile[dMissile[x][y] - 1];
...
void __fastcall DrawDeadPlayer(int x, int y, int sx, int sy, int a5, int dir, BOOL clipped)
{
	int i, px, py, nCel;
	PlayerStruct *p;
	BYTE *pCelBuff;
	...
	for (i = 0; i < MAX_PLRS; i++) {
		...
		if (p->plractive && !p->_pHitPoints && p->plrlevel == (unsigned char)currlevel && p->WorldX == x && p->WorldY == y) {
			...
			py = sy + p->_pyoff;
			DrawPlrProc(i, x, y, px, py, p->_pAnimData, p->_pAnimFrame, p->_pAnimWidth, a5, dir);
		}
	}
}

void __fastcall DrawPlayer(int pnum, int x, int y, int px, int py, BYTE *pCelBuff, int nCel, int nWidth, int a9, int dir)
{
	int l;
	DWORD *pFrameTable;
	...
	if (dFlags[x][y] & DFLAG_LIT || plr[myplr]._pInfraFlag || !setlevel && !currlevel) {
		...
		if (pnum == pcursplr)
			Cl2DecodeFrm2(165, px, py, pCelBuff, nCel, nWidth, a9, dir);
		if (pnum == myplr) {
			Cl2DecodeFrm1(px, py, pCelBuff, nCel, nWidth, a9, dir);
			...
			Cl2DecodeFrm3(px, py, pCelBuff, nCel, nWidth, a9, dir, 1);
			if (plr[pnum].pManaShield)
				Cl2DecodeFrm3(
				    px + plr[pnum]._pAnimWidth2 - misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
				    py,
			...
			if (light_table_index < 5)
				...
				light_table_index -= 5;
			Cl2DecodeLightTbl(px, py, pCelBuff, nCel, nWidth, a9, dir);
			if (plr[pnum].pManaShield)
				Cl2DecodeLightTbl(
				    px + plr[pnum]._pAnimWidth2 - misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
				    py,
...
void __fastcall DrawClippedPlayer(int pnum, int x, int y, int px, int py, BYTE *pCelBuff, int nCel, int nWidth, int a9, int dir)
{
	int l;
	DWORD *pFrameTable;
	...
	if (dFlags[x][y] & DFLAG_LIT || plr[myplr]._pInfraFlag) {
		...
		if (pnum == pcursplr)
			Cl2DecodeClrHL(165, px, py, pCelBuff, nCel, nWidth, a9, dir);
		if (pnum == myplr) {
			Cl2DecodeFrm4(px, py, pCelBuff, nCel, nWidth, a9, dir);
			if (plr[pnum].pManaShield)
				Cl2DecodeFrm4(
				    px + plr[pnum]._pAnimWidth2 - misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
				    py,
			...
			Cl2DecodeFrm5(px, py, pCelBuff, nCel, nWidth, a9, dir, 1);
			if (plr[pnum].pManaShield)
				Cl2DecodeFrm5(
				    px + plr[pnum]._pAnimWidth2 - misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
				    py,
			...
			if (light_table_index < 5)
				...
				light_table_index -= 5;
			Cl2DecodeFrm6(px, py, pCelBuff, nCel, nWidth, a9, dir);
			if (plr[pnum].pManaShield)
				Cl2DecodeFrm6(
				    px + plr[pnum]._pAnimWidth2 - misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
				    py,
...
void __fastcall scrollrt_draw_lower(int x, int y, int sx, int sy, int chunks, int some_flag)
{
	int i, j;
	BYTE *dst;
	WORD *pMap;
...
void __fastcall scrollrt_draw_clipped_dungeon(char *a1, int x, int y, int sx, int sy, int a6)
{
	int v6;             // eax
	char v7;            // bl
	char v8;            // cl
	...
	if (visiondebug && v50 & DFLAG_LIT)
		Cel2DecodeHdrOnly((BYTE *)dst_buf, (BYTE *)pSquareCel, 1, 64, 0, DIR_OMNI);
	if (MissilePreFlag && v50 & DFLAG_MISSILE)
		DrawClippedMissile(a1a, y, sx, sy, 0, DIR_OMNI, TRUE);
	if (light_table_index < lightmax) {
		if (v7) {
			v11 = &dead[(v7 & 0x1F) - 1];
			v12 = (int *)v11->_deadData[(v7 >> 5) & 7];
			v13 = sx - v11->_deadWidth2;
			if (v12) {
				v14 = v11->_deadFrame;
				if (v14 >= 1 && (unsigned int)*v12 <= 0x32 && v14 <= *v12) {
					v15 = v11->_deadtrans;
					if (v15)
						Cl2DecodeFrm5(v13, sy, (BYTE *)v12, v14, v11->_deadWidth, 0, DIR_OMNI, v15);
					else
						Cl2DecodeFrm6(v13, sy, (BYTE *)v12, v14, v11->_deadWidth, 0, DIR_OMNI);
				}
			}
		}
		if (v47)
			DrawClippedObject(a1a, y, sx, sy, 1, 0, DIR_OMNI);
	}
	if (v49) {
		v16 = &item[v49 - 1];
		if (!v16->_iPostDraw && (unsigned char)v49 <= MAXITEMS) {
			...
			if (v17) {
				...
				if (v18 >= 1 && *(_DWORD *)v17 <= 0x32u && v18 <= *(_DWORD *)v17) {
					v19 = sx - v16->_iAnimWidth2;
					if (v49 - 1 == pcursitem)
						CelDrawHdrClrHL(ICOL_BLUE, v19, sy, (BYTE *)v17, v16->_iAnimFrame, v16->_iAnimWidth, 0, DIR_OMNI);
					Cel2DecodeHdrLight(v19, sy, v16->_iAnimData, v16->_iAnimFrame, v16->_iAnimWidth, 0, DIR_OMNI);
				}
			}
		}
	}
	if (v50 & DFLAG_PLAYER) {
		...
		if (v20 < 4) {
			...
			DrawClippedPlayer(
			    ...
			    a1a,
			    y - 1,
			    sx + v21->_pxoff - v21->_pAnimWidth2,
			    sy + v21->_pyoff,
			    v21->_pAnimData,
			    v21->_pAnimFrame,
			    v21->_pAnimWidth,
			...
			if (a6) {
				...
				if (v22) {
					if (v22 == 2)
						scrollrt_draw_clipped_e_flag((BYTE *)dst_buf - 12384, a1a - 2, y + 1, sx - 96, sy - 16);
					scrollrt_draw_clipped_e_flag((BYTE *)dst_buf - 64, a1a - 1, y + 1, sx - 64, sy);
				}
			}
		}
	}
	if (v50 & DFLAG_MONSTER && (v50 & DFLAG_LIT || plr[myplr]._pInfraFlag) && v41 < 0) {
		...
		if ((unsigned int)(-1 - v41) < MAXMONSTERS) {
			...
			if (!(v24->_mFlags & MFLAG_HIDDEN)) {
				...
				if (v25) {
					v26 = sy + v24->_myoff;
					v27 = sx + v24->_mxoff - v25->width2;
					if (v23 == pcursmonst) {
						Cl2DecodeClrHL(233, v27, v26, v24->_mAnimData, v24->_mAnimFrame, v25->width, 0, DIR_OMNI);
						v23 = draw_monster_num;
					}
					DrawClippedMonster(a1a, y, v27, v26, v23, 0, DIR_OMNI);
					if (a6 && v24->_meflag)
						scrollrt_draw_clipped_e_flag((BYTE *)dst_buf - 64, a1a - 1, y + 1, sx - 64, sy);
				}
			}
	...
	if (v50 & 4)
		DrawDeadPlayer(a1a, y, sx, sy, 0, DIR_OMNI, 1);
	if (v48 > 0) {
		v28 = v48 - 1;
		if (v28 < 4) {
			v29 = &plr[v28];
			DrawClippedPlayer(
			    ...
			    a1a,
			    y,
			    sx + v29->_pxoff - v29->_pAnimWidth2,
			    sy + v29->_pyoff,
			    v29->_pAnimData,
			    v29->_pAnimFrame,
			    v29->_pAnimWidth,
	...
	if (v40 > 0 && (v50 & 0x40 || plr[myplr]._pInfraFlag)) {
		...
		if ((unsigned int)(v40 - 1) < MAXMONSTERS) {
			...
			if (!(v32->_mFlags & MFLAG_HIDDEN)) {
				...
				if (v33) {
					v34 = sy + v32->_myoff;
					v35 = sx + v32->_mxoff - v33->width2;
					if (v31 == pcursmonst) {
						Cl2DecodeClrHL(233, v35, v34, v32->_mAnimData, v32->_mAnimFrame, v33->width, 0, DIR_OMNI);
						v31 = draw_monster_num;
					}
					DrawClippedMonster(a1a, y, v35, v34, v31, 0, DIR_OMNI);
					if (a6 && v32->_meflag)
						scrollrt_draw_clipped_e_flag((BYTE *)dst_buf - 64, a1a - 1, y + 1, sx - 64, sy);
				}
			}
	...
	if (v50 & 1)
		DrawClippedMissile(a1a, y, sx, sy, 0, DIR_OMNI, FALSE);
	if (v47 && light_table_index < lightmax)
		DrawClippedObject(a1a, y, sx, sy, 0, 0, DIR_OMNI);
	if (v49) {
		v36 = &item[v49 - 1];
		if (v36->_iPostDraw) {
			if ((unsigned char)v49 <= MAXITEMS) {
				...
				if (v37) {
					...
					if (v38 >= 1 && *(_DWORD *)v37 <= 0x32u && v38 <= *(_DWORD *)v37) {
						v39 = sx - v36->_iAnimWidth2;
						if (v49 - 1 == pcursitem)
							CelDrawHdrClrHL(ICOL_BLUE, v39, sy, (BYTE *)v37, v36->_iAnimFrame, v36->_iAnimWidth, 0, 8);
						Cel2DecodeHdrLight(
						    v39,
						    sy,
						    v36->_iAnimData,
...
void __fastcall DrawClippedMonster(int x, int y, int mx, int my, int m, int a6, int dir)
{
	int nCel;
	char trans;
	BYTE *pCelBuff;
	...
	if (!(dFlags[x][y] & DFLAG_LIT)) {
		Cl2DecodeFrm5(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir, 1);
	} else {
		trans = 0;
		...
		if (plr[myplr]._pInfraFlag && light_table_index > 8)
			trans = 1;
		if (trans)
			Cl2DecodeFrm5(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir, trans);
		else
			Cl2DecodeFrm6(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir);
	}
}
// 69BEF8: using guessed type int light_table_index;
...
void __fastcall scrollrt_draw_clipped_e_flag(BYTE *pBuff, int x, int y, int sx, int sy)
{
	...
	scrollrt_draw_clipped_dungeon((char *)pBuff, x, y, sx, sy, 0);
...
void __fastcall scrollrt_draw_lower_2(int x, int y, int sx, int sy, int chunks, int a6, int some_flag)
{
	int i, j, z;
	BYTE *dst;
	WORD *pMap;
	...
	if (some_flag) {
		if (y >= 0 && y < MAXDUNY && x >= 0 && x < MAXDUNX) {
			...
			if (level_piece_id != 0) {
				...
				if (z < 8) {
					scrollrt_draw_clipped_dungeon_2((char *)&gpBuffer[sx + screen_y_times_768[sy] - 768 * 16 * z], x, y, a6, z, sx, sy, 0);
				}
			}
		}
		x++;
	...
	while (j != 0) {
		...
		if (y < MAXDUNY && x >= 0) {
			...
			if (level_piece_id != 0) {
				...
				if (z < 8) {
					scrollrt_draw_clipped_dungeon_2((char *)&gpBuffer[sx + screen_y_times_768[sy] - 768 * 32 * (a6 + 1)], x, y, a6, z, sx, sy, 1);
				}
			}
		}
		x++;
...
void __fastcall scrollrt_draw_clipped_dungeon_2(char *buffer, int x, int y, int a4, int a5, int sx, int sy, int me_flag)
{
	...
	if (visiondebug && v53 & DFLAG_LIT)
		Cel2DecodeHdrOnly((BYTE *)dst_buf, (BYTE *)pSquareCel, 1, 64, a5, DIR_OMNI);
	if (MissilePreFlag && v53 & DFLAG_MISSILE) {
		v13 = sx;
		DrawClippedMissile(a1, y, sx, sy, a5, DIR_OMNI, TRUE);
	} else {
		v13 = sx;
	}
	if (light_table_index < lightmax) {
		if (v9) {
			...
			if (v15) {
				...
				if (v17 >= 1 && (unsigned int)*v15 <= 0x32 && v17 <= *v15) {
					v18 = v14->_deadtrans;
					if (v18)
						Cl2DecodeFrm5(v16, sy, (BYTE *)v15, v17, v14->_deadWidth, a5, DIR_OMNI, v18);
					else
						Cl2DecodeFrm6(v16, sy, (BYTE *)v15, v17, v14->_deadWidth, a5, DIR_OMNI);
				}
			}
		}
		if (v50)
			DrawClippedObject(a1, y, v13, sy, 1, a5, DIR_OMNI);
	}
	if (v52) {
		v19 = &item[v52 - 1];
		...
		if ((unsigned int)(-1 - v44) < MAXMONSTERS) {
			...
			if (!(v27->_mFlags & MFLAG_HIDDEN)) {
				...
				if (v28) {
					...
					if (v26 == pcursmonst) {
						Cl2DecodeClrHL(233, v30, v29, v27->_mAnimData, v27->_mAnimFrame, v28->width, a5, DIR_OMNI);
						v26 = draw_monster_num;
					}
					DrawClippedMonster(a1, y, v30, v29, v26, a5, DIR_OMNI);
	...
	if (v53 & DFLAG_DEAD_PLAYER)
		DrawDeadPlayer(a1, y, v13, sy, a5, DIR_OMNI, 1);
	if (v51 > 0) {
		v31 = v51 - 1;
		if (v31 < 4) {
			v32 = &plr[v31];
		...
		if ((unsigned int)(v43 - 1) < MAXMONSTERS) {
			...
			if (!(v35->_mFlags & MFLAG_HIDDEN)) {
				...
				if (v36) {
					...
					if (v34 == pcursmonst) {
						Cl2DecodeClrHL(233, v38, v37, v35->_mAnimData, v35->_mAnimFrame, v36->width, a5, DIR_OMNI);
						v34 = draw_monster_num;
					}
					DrawClippedMonster(a1, y, v38, v37, v34, a5, DIR_OMNI);
	...
	if (v53 & DFLAG_MISSILE)
		DrawClippedMissile(a1, y, v13, sy, a5, DIR_OMNI, FALSE);
	if (v50 && light_table_index < lightmax)
		DrawClippedObject(a1, y, v13, sy, 0, a5, DIR_OMNI);
	if (v52) {
		v39 = &item[v52 - 1];
		if (v39->_iPostDraw) {
...
void __fastcall scrollrt_draw_dungeon(BYTE *pBuff, int sx, int sy, int a4, int dir, int dx, int dy, int me_flag)
{
	...
	if (visiondebug && (bFlag & 0x40)) {
		CelDecodeHdrOnly(pBuff, (BYTE *)pSquareCel, 1, 64, 0, dir);
	}
	if (MissilePreFlag && (bFlag & 1)) {
		DrawMissile(sx, sy, dx, dy, 0, dir, 1);
	}
	if (light_table_index < lightmax) {
		if (bDead != 0) {
			...
			if (pCelBuff != NULL) {
				...
				if (nCel >= 1 && pFrameTable[0] <= 50 && nCel <= (int)pFrameTable[0]) {
					if (pDeadGuy->_deadtrans != 0) {
						Cl2DecodeFrm3(px, dy, pDeadGuy->_deadData[dd], pDeadGuy->_deadFrame, pDeadGuy->_deadWidth, 0, dir, pDeadGuy->_deadtrans);
					} else {
						Cl2DecodeLightTbl(px, dy, pDeadGuy->_deadData[dd], pDeadGuy->_deadFrame, pDeadGuy->_deadWidth, 0, dir);
					}
				} else {
		...
		if (bObj != 0) {
			DrawObject(sx, sy, dx, dy, 1, 0, dir);
		}
	}
	if (bItem != 0) {
		pItem = &item[bItem - 1];
				...
				if (pCelBuff != NULL) {
					...
					if (nCel >= 1 && pFrameTable[0] <= 50 && nCel <= (int)pFrameTable[0]) {
						...
						if (bItem - 1 == pcursitem) {
							CelDecodeClr(181, px, dy, pItem->_iAnimData, pItem->_iAnimFrame, pItem->_iAnimWidth, 0, dir);
						}
						CelDecodeHdrLightOnly(px, dy, pItem->_iAnimData, pItem->_iAnimFrame, pItem->_iAnimWidth, 0, dir);
					} else {
	...
	if (bFlag & 0x20) {
		...
		if ((DWORD)p < MAX_PLRS) {
			...
			DrawPlayer(p, sx, sy - 1, px, py, pPlayer->_pAnimData, pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, 0, dir);
			if (me_flag && pPlayer->_peflag != 0) {
				if (pPlayer->_peflag == 2) {
					scrollrt_draw_e_flag(pBuff - (768 * 16 + 96), sx - 2, sy + 1, a4, dir, dx - 96, dy - 16);
				}
				scrollrt_draw_e_flag(pBuff - 64, sx - 1, sy + 1, a4, dir, dx - 64, dy);
			}
		} else {
			...
			if (!(pMonster->_mFlags & 1)) {
				if (pMonster->MType != NULL) {
					...
					if (draw_monster_num == pcursmonst) {
						Cl2DecodeFrm2(233, px, py, pMonster->_mAnimData, pMonster->_mAnimFrame, pMonster->MType->width, 0, dir);
					}
					DrawMonster(sx, sy, px, py, draw_monster_num, 0, dir);
	...
	if (bFlag & 4) {
		DrawDeadPlayer(sx, sy, dx, dy, 0, dir, 0);
	}
	if (bPlr > 0) {
		p = bPlr - 1;
		if ((DWORD)p < MAX_PLRS) {
			...
			px = dx + pPlayer->_pxoff - pPlayer->_pAnimWidth2;
			py = dy + pPlayer->_pyoff;
			DrawPlayer(p, sx, sy, px, py, pPlayer->_pAnimData, pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, 0, dir);
			if (me_flag && pPlayer->_peflag != 0) {
				if (pPlayer->_peflag == 2) {
					scrollrt_draw_e_flag(pBuff - (768 * 16 + 96), sx - 2, sy + 1, a4, dir, dx - 96, dy - 16);
				}
	...
	if (bFlag & 1) {
		DrawMissile(sx, sy, dx, dy, 0, dir, 0);
	}
	if (bObj != 0 && light_table_index < lightmax) {
		DrawObject(sx, sy, dx, dy, 0, 0, dir);
	}
	if (bItem != 0) {
		pItem = &item[bItem - 1];
		if (pItem->_iPostDraw) {
	...
	if (bArch != 0) {
		...
		CelDecodeHdrLightTrans(pBuff, (BYTE *)level_special_cel, bArch, 64, 0, dir);
	}
}
// 4B8CC0: using guessed type char pcursitem;
// 525720: using guessed type int visiondebug;
...
void __fastcall DrawMonster(int x, int y, int mx, int my, int m, int a6, int dir)
{
	int nCel;
	char trans;
	BYTE *pCelBuff;
	...
	if (!(dFlags[x][y] & DFLAG_LIT)) {
		Cl2DecodeFrm3(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir, 1);
	} else {
		trans = 0;
		if (monster[m]._uniqtype)
			trans = monster[m]._uniqtrans + 4;
		...
		if (plr[myplr]._pInfraFlag && light_table_index > 8)
			trans = 1;
		if (trans)
			Cl2DecodeFrm3(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir, trans);
		else
			Cl2DecodeLightTbl(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame, monster[m].MType->width, a6, dir);
	}
}
// 69BEF8: using guessed type int light_table_index;
...
void __fastcall scrollrt_draw_e_flag(BYTE *pBuff, int x, int y, int a4, int dir, int sx, int sy)
{
	int i, lti_old, cta_old, lpi_old;
	BYTE *dst;
	WORD *pMap;
	...
	scrollrt_draw_dungeon(pBuff, x, y, a4, dir, sx, sy, 0);
